...
class DATA:
    ...
    def object (request, id, cls, format = 'json'):
        ...
        in_regexs, ex_regexs, rs_regexs = DATA.parse (request)
        ...
        ##       cluded from serialization, default: [], below include or
        ##       resolve!
        ##
    ...
    set2xml_string = staticmethod (set2xml_string)

    def set2json_string (qs):

        return serializers.serialize ('json', qs)

    set2json_string = staticmethod (set2json_string)
        ...
        try:
            ...
            if (format == 'xml') :
                js_string = '<?xml version="1.0" encoding="utf-8"?>\n' \
                    ...
                    .join (map (DATA.set2xml_string, querysets))
                
            if (format == 'json'):
                js_string = '[%s]' % ', ' \
                    .join (map (DATA.set2json_string, querysets))
